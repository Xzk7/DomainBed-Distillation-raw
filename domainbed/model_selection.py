...
class OracleSelectionMethod(SelectionMethod):
    ...
    def run_acc(self, run_records):
        ...
        for i in itertools.count():
            if f'env{i}_out_acc' not in chosen_record:
                break
            if i != test_env:
                train_keys.append(f'env{i}_in_acc')
                train_keys.append(f'env{i}_out_acc')
        output = {
            'step': chosen_record['step'],
            'val_acc':  chosen_record[test_out_acc_key],
            'test_in_acc': chosen_record[test_in_acc_key],
            'test_out_acc': chosen_record[test_out_acc_key]
        }
        train_dict = dict()
        for key in train_keys:
            train_dict[key] = chosen_record[key]
        output.update(train_dict)
        return output
...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, record):
        ...
        # print(test_env, record['args']['test_envs'])
        val_env_keys = []
        train_keys = []
        for i in itertools.count():
        ...
        output = {
            ...
            'test_in_acc': record[test_in_acc_key],
            'test_out_acc': record[test_out_acc_key],
        }
        train_dict = dict()
        for key in train_keys:
            train_dict[key] = record[key]
        output.update(train_dict)
        return output
...
class LeaveOneOutSelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, records):
        ...
        # print(test_env, test_records[0]['args']['test_envs'])
        n_envs = 0
        train_keys = []
        for i in itertools.count():
        ...
        output = {
            ...
            'val_acc': val_acc,
            'test_in_acc': test_records[0]['env{}_in_acc'.format(test_env)],
            'test_out_acc': test_records[0]['env{}_out_acc'.format(test_env)],
        }
        train_dict = dict()
        for key in train_keys:
            train_dict[key] = test_records[0][key]
        output.update(train_dict)
        return output
