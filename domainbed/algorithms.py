...
class ERMDistill(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams, args):
        super(ERMDistill, self).__init__(input_shape, num_classes, num_domains, hparams)
        records = reporting.load_records(args.input_dir)
        assert args.feat_layers in ["stem_block", "block"]
        print("Total records:", len(records))

        hparams_teacher = hparams.copy()
        hparams_teacher['resnet18'] = False
        print(hparams, hparams_teacher)
        records = records.filter(
            lambda r:
            r['args']['dataset'] == args.dataset and
            r['args']['algorithm'] == args.algorithm.replace("Distill", "") and
            r['args']['test_envs'] == args.test_envs and
            r['args']['seed'] == args.seed and
            r['hparams'] == hparams_teacher
        )
        # print(len(records), len(records))
        output_dir = records[0]['args']['output_dir']
        ckpt = os.path.join(output_dir, 'model.pkl')
        hparams_teacher["model"] = "resnet50"
        hparams_teacher["pretrained"] = True
        self.teacher_featurizer = URFeaturizer(
            input_shape, hparams_teacher, freeze="all", feat_layers=args.feat_layers
        )
        self.teacher_featurizer.load_state_dict(torch.load(ckpt), strict=False)
        self.teacher_featurizer.eval()
        self.teacher_classifier = nn.Linear(self.teacher_featurizer.n_outputs, num_classes)
        self.teacher_classifier.load_state_dict(torch.load(ckpt), strict=False)
        self.teacher_classifier.eval()
        self.num_classes = num_classes
        hparams["model"] = "resnet18"
        hparams["pretrained"] = True
        self.featurizer = URFeaturizer(
            input_shape, self.hparams, feat_layers=args.feat_layers
        )
        self.input_shape = input_shape
        self.lambda_mgd = args.lambda_mgd
        self.weight_mgd = args.weight_mgd
        self.weight_wsld = args.weight_wsld
        self.distillation_type = args.distillation_type
        self.T = 2
        self.prepare_generation()
        param_align = list()
        param_generation = list()
        for align, generation in zip(self.alignlist, self.generationlist):
            if align:
                param_align += list(align.parameters())
            param_generation += list(generation.parameters())
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.optimizer = torch.optim.Adam(
            list(self.network.parameters())+param_generation+param_align,
            lr=self.hparams["lr"],
            weight_decay=self.hparams["weight_decay"],
        )
    ...
    def prepare_generation(self):
        self.alignlist = []
        self.generationlist = []
        input = torch.randn((2,) + self.input_shape)
        _, inter_featt = self.teacher_featurizer(input, ret_feats=True)
        _, inter_feats = self.featurizer(input, ret_feats=True)
        for feat, feas in zip(inter_featt, inter_feats):
            student_channels = feas.size()[1]
            teacher_channels = feat.size()[1]
            if student_channels != teacher_channels:
                self.alignlist.append(nn.Conv2d(student_channels, teacher_channels, kernel_size=1, stride=1, padding=0))
            else:
                self.alignlist.append(None)
            self.generationlist.append(nn.Sequential(
    ...
    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])
        outs, inter_feats = self.featurizer(all_x, ret_feats=True)
        outt, inter_featt = self.teacher_featurizer(all_x, ret_feats=True)

        fc_s = self.classifier(outs)
        fc_t = self.teacher_classifier(outt)
        # loss = 0.
        # for feas, feat, align, generation in zip(inter_feats, inter_featt, self.alignlist, self.generationlist):
        #     loss += self.weight_mgd * self.get_dis_loss(feas, feat, align, generation)
        if self.distillation_type == "mgd":
            loss = self.weight_mgd * self.get_dis_loss(inter_feats[-1], inter_featt[-1], self.alignlist[-1],
                                                       self.generationlist[-1])
        elif self.distillation_type == "wsld":
            loss = self.weight_wsld * self.get_logits_loss(fc_t, fc_s, all_y)
        else:
            loss = self.weight_mgd * self.get_dis_loss(inter_feats[-1], inter_featt[-1], self.alignlist[-1], self.generationlist[-1]) + self.weight_wsld * self.get_logits_loss(fc_t, fc_s, all_y)
        loss += F.cross_entropy(self.predict(all_x), all_y)
    ...
    def get_logits_loss(self, fc_t, fc_s, label):
    ...
    def get_dis_loss(self, preds_S, preds_T, align, generation):
        ...
        if align:
            align.to(device)
            preds_S = align(preds_S)

        mat = torch.rand((N,1,H,W)).to(device)
        mat = torch.where(mat>1-self.lambda_mgd, 0, 1).to(device)

        masked_fea = torch.mul(preds_S, mat)
        new_fea = generation(masked_fea)
